...
export class ChipPin<T> implements WireEntity<T> {
	...
	calculateOutletPosition() {
		...
		this.outletPosition
			.copy(this.position)
			.addVector(dir.multScalar(DEFUALTS.PIN_OUTLET_LINE_LENGTH));
	}
	...
	calculateBound() {
		...
		const end = this.outletPosition
			.clone()
			.addVector(dirVector.clone().multScalar(DEFUALTS.PIN_OUTLET_SIZE));
	...
	select(pointCollider: PointCollider) {
		if (this.isCollidingOutlet(pointCollider) || this.isCollidingOutletLine(pointCollider)) {
