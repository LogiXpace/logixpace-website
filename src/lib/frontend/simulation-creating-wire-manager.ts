...
export class SimulationCreatingWireManager<T> {
	...
	createAndSplitOn(closestPoint: Vector2D, wire: Wire<T>) {
		if (this.wirePointPending !== undefined) {
			...
			this.entityManager.adapter.connect(
				connectedWirePending.start.pinId,
				connectedWirePending.end.pinId
			);
		} else {
	...
	create() {
		...
		wirePoint.pinId = this.entityManager.adapter.createPin(
			this.entityManager.adapter.getPowerState(start.pinId)
		);
