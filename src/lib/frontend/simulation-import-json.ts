...
export function importJSON<T>(json: string, simulationContext: SimulationContext<T>) {
	...
	const worldPosition = simulationContext.screenVectorToWorldVector(
		simulationContext.mouseInput.movePosition
	);
	...
	for (let i = 0; i < inputsSerialized.length; i++) {
		...
		const input = new Input({
			namedPin,
			position: new Vector2D(inputSerialized.position.x, inputSerialized.position.y).addVector(
				delta
			),
			color: new RGB(
				inputSerialized.color.red,
				inputSerialized.color.green,
				inputSerialized.color.blue
			)
		});
	...
	for (let i = 0; i < outputsSerialized.length; i++) {
		...
		const output = new Output({
			namedPin,
			position: new Vector2D(outputSerialized.position.x, outputSerialized.position.y).addVector(
				delta
			),
			color: new RGB(
				outputSerialized.color.red,
				outputSerialized.color.green,
				outputSerialized.color.blue
			)
		});
	...
	for (let i = 0; i < wirePointsSerialized.length; i++) {
		...
		const wirePoint = new WirePoint({
			...
			position: new Vector2D(
				wirePointSerialized.position.x,
				wirePointSerialized.position.y
			).addVector(delta)
		});
	...
	for (let i = 0; i < chipsSerialized.length; i++) {
		...
		const outputPins = chipSerialized.outputPinIndices.map((pinIndex) =>
			addChipPin(pinIndex, false)
		);
		...
		const chip = new Chip({
			...
			color: new RGB(
				chipSerialized.color.red,
				chipSerialized.color.green,
				chipSerialized.color.blue
			),
			type: chipSerialized.type,
			textWidth,
			simulationContext,
			id: simulationContext.adapter.createChip(
				chipSerialized.type,
	...
	for (let i = 0; i < wiresSerialized.length; i++) {
		...
		if (startType === 'input') {
			...
			startPosition = (mapChipPins.get(wireSerialized.startEntityIndex) as ChipPin<T>)
				.outletPosition;
		}
